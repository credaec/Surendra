        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-5">
                <div className="flex items-center mb-6">
                    <Briefcase className="h-4 w-4 text-slate-400 dark:text-slate-500 mr-2" />
                    <h3 className="font-semibold text-slate-900 dark:text-white">Project Breakdown</h3>
                </div>
                {projectStats.length === 0 ? (
                    <p className="text-sm text-slate-500 dark:text-slate-400">No data available for this period.</p>
                ) : (
                        ...
                        {projectStats.map((project) => (
                            <div key={project.name}>
                                <div className="flex justify-between text-sm mb-1">
                                    ...
                                    <span className="text-slate-500 dark:text-slate-400">{project.hours.toFixed(1)}h ({project.percent.toFixed(0)}%)</span>
                                </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-5">
                <div className="flex items-center mb-6">
                    <Folder className="h-4 w-4 text-slate-400 dark:text-slate-500 mr-2" />
                ...
                {categoryStats.length === 0 ? (
                    ...
                    <div className="space-y-4">
                        {categoryStats.map((cat) => (
                            <div key={cat.name} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-800">
                                <div>
                                    <div className="font-medium text-slate-900 dark:text-white text-sm">{cat.name}</div>
                                </div>
                                <div className="text-sm font-bold text-slate-700 dark:text-slate-300">{cat.hours.toFixed(1)}h</div>
                            </div>
                        ))}
                    </div>
                )}
