...
const TeamPage: React.FC = () => {
    ...
    return (
        <div className="max-w-[1600px] mx-auto p-8">
            ...
            {/* Filters */}
            <div className="bg-white rounded-t-xl border border-slate-200 border-b-0 p-4 flex items-center justify-between relative z-10">
                ...
                <div className="relative">
                    <button
                        ...
                        className={`flex items-center px-3 py-2 text-sm font-medium border rounded-lg transition-colors ${filterRole !== 'ALL' ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                    >
                        <Filter className={`h-4 w-4 mr-2 ${filterRole !== 'ALL' ? 'text-blue-500' : 'text-slate-400'}`} />
                        Filter
                        {filterRole !== 'ALL' && <span className="ml-2 bg-blue-200 text-blue-800 text-[10px] px-1.5 py-0.5 rounded-full">{filterRole}</span>}
                    </button>
                        ...
                        <>
                            ...
                            <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-lg shadow-lg border border-slate-100 py-1 z-50 animate-in fade-in zoom-in-95 duration-200">
                                ...
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-2 text-sm hover:bg-slate-50 ${filterRole === 'ALL' ? 'text-blue-600 font-medium bg-blue-50/50' : 'text-slate-700'}`}
                                >
                                    All Roles
                                </button>
                                <button
                                    onClick={() => { setFilterRole('ADMIN'); setIsFilterOpen(false); }}
                                    className={`w-full text-left px-4 py-2 text-sm hover:bg-slate-50 ${filterRole === 'ADMIN' ? 'text-blue-600 font-medium bg-blue-50/50' : 'text-slate-700'}`}
                                >
                                    Admin
                                </button>
                                <button
                                    onClick={() => { setFilterRole('EMPLOYEE'); setIsFilterOpen(false); }}
                                    className={`w-full text-left px-4 py-2 text-sm hover:bg-slate-50 ${filterRole === 'EMPLOYEE' ? 'text-blue-600 font-medium bg-blue-50/50' : 'text-slate-700'}`}
                                >
                                    Employee
                                </button>
                            </div>
                            ...
                            <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td className="px-6 py-4 text-right relative">
                                    <button
                                        ...
                                        className="p-2 text-slate-400 hover:text-slate-600 rounded-lg hover:bg-slate-100 opacity-0 group-hover:opacity-100 transition-all"
                                    >
                                        <MoreHorizontal className="h-5 w-5" />
                                    </button>

                                    {activeMenuId === user.id && (
                                        <div className="absolute right-8 top-8 w-48 bg-white rounded-lg shadow-lg border border-slate-100 py-1 z-50 animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                                            <button
                                                onClick={() => {
                                                    setEditingUser(user);
