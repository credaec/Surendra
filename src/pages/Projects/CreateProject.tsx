...
const CreateProjectPage: React.FC = () => {
    ...
    const [clients, setClients] = useState<any[]>(backendService.getClients());
    const [projectRoles, setProjectRoles] = useState<string[]>([]);

    useEffect(() => {
        // Fetch users who are EMPLOYEEs or ADMINs
        const users = backendService.getUsers().filter(u => u.role === 'EMPLOYEE' || u.role === 'ADMIN');
        ...
        // Fetch project roles
        setProjectRoles(backendService.getProjectRoles());
    }, []);
    ...
    const handleCreateProject = async () => {
        try {
            ...
            // Process Documents (Mock Upload)
            const projectDocuments = uploadedFiles.map(file => ({
                id: crypto.randomUUID(),
                name: file.name,
                size: (file.size / 1024 / 1024).toFixed(2) + ' MB',
                type: file.name.split('.').pop()?.toUpperCase() || 'FILE',
                uploadedAt: new Date().toISOString(),
                uploadedBy: 'CurrentUser' // Mock
            }));
        ...
        <div className="max-w-5xl mx-auto pb-12 animate-in fade-in duration-500">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-8 min-h-[400px]">
                                    ...
                                    {formData.rateLogic === 'GLOBAL_PROJECT_RATE' && formData.billingMode === 'HOURLY_RATE' && (
                                        <div className="mt-4">
                                            ...
                                            <div className="relative max-w-xs">
                                                ...
                                                <input type="number" className="w-full pl-8 rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:ring-blue-500" placeholder="0.00"
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <label className="flex items-center p-4 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer">
                                            ...
                                            />
                                            <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Require Notes on Entry</span>
                                        </label>
                                        <label className="flex items-center p-4 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer">
                                            <input type="checkbox" checked={formData.entryRules?.proofRequired}
                                                ...
                                                className="h-5 w-5 text-blue-600 rounded focus:ring-blue-500 mr-3"
                                            />
                                            <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Require Attachment / Proof</span>
                                        </label>
                                        <label className="flex items-center p-4 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer">
                                            <input type="checkbox" checked={formData.entryRules?.allowBackdatedEntry}
                                                ...
                                                className="h-5 w-5 text-blue-600 rounded focus:ring-blue-500 mr-3"
                                            />
                                            <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Allow Backdated Entries</span>
                                        </label>
                                        <label className="flex items-center p-4 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer">
                                            <input type="checkbox" checked={formData.entryRules?.allowFutureEntry}
                                                ...
                                                className="h-5 w-5 text-blue-600 rounded focus:ring-blue-500 mr-3"
                                            />
                                            <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Allow Future Entries</span>
                                        </label>
                                    </div>

                                    <div className="mt-4">
                                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Minimum Time Unit (Minutes)</label>
                                        <select className="w-full max-w-xs rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:ring-blue-500"
                        ...
                        {activeTab === 'documents' && (
                            <div className="space-y-6">
                                <div
                                    className="text-center py-12 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer group"
                                    onClick={() => document.getElementById('file-upload')?.click()}
                                >
                                    <div className="h-16 w-16 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform">
                                        <UploadCloud className="h-8 w-8" />
                                    </div>
                                    <h3 className="text-lg font-medium text-slate-900 dark:text-white">Upload Project Documents</h3>
                                    <p className="text-slate-500 dark:text-slate-400 mt-1 mb-6">Drag and drop files here, or click to browse</p>
                                    <button className="px-6 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                                        Browse Files
                                    </button>
                                    <input
                                        type="file"
                                    ...
                                    />
                                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-4">Supported: PDF, DWG, DOCX, ZIP</p>
                                </div>

                                {uploadedFiles.length > 0 && (
                                    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                                        <div className="px-4 py-3 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                                            <h4 className="text-sm font-medium text-slate-700 dark:text-slate-300">Selected Files ({uploadedFiles.length})</h4>
                                        </div>
                                        <ul className="divide-y divide-slate-200 dark:divide-slate-700">
                                            {uploadedFiles.map((file, idx) => (
                                                <li key={idx} className="px-4 py-3 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                                    <div className="flex items-center">
                                                        <FileText className="h-5 w-5 text-slate-400 mr-3" />
                                                        <div>
                                                            <p className="text-sm font-medium text-slate-900 dark:text-white truncate max-w-[300px]">{file.name}</p>
